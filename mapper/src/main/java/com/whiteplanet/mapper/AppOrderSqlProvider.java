...
public class AppOrderSqlProvider {
    ...
    public String insertSelective(AppOrder record) {
        ...
        if (record.getTransFree() != null) {
            VALUES("is_trans_free", "#{isTransFree,jdbcType=INTEGER}");
        }

        if (record.getReceiveFree() != null) {
            VALUES("is_receive_free", "#{isReceiveFree,jdbcType=INTEGER}");
        }

        if (record.getPatientEvaluated() !=null) {
            VALUES("is_patient_evaluated", "#{isPatientEvaluated,jdbcType=INTEGER}");
        }

        if (record.getPaid() != null) {
            VALUES("is_paid", "#{isPaid,jdbcType=INTEGER}");
        }
        ...
        if (record.getCancelType() != null) {
            VALUES("cancel_type", "#{cancelType,jdbcType=INTEGER}");
        }
    ...
    public String updateByPrimaryKeySelective(AppOrder record) {
        ...
        if (record.getTransFree() != null) {
            SET("is_trans_free = #{isTransFree,jdbcType=INTEGER}");
        }

        if (record.getReceiveFree() != null) {
            SET("is_receive_free = #{isReceiveFree,jdbcType=INTEGER}");
        }

        if (record.getPaid() != null) {
            SET("is_paid = #{isPaid,jdbcType=INTEGER}");
        }

        if (record.getPatientEvaluated() != null) {
            SET("is_patient_evaluated = #{isPatientEvaluated,jdbcType=INTEGER}");
        }
        ...
        if (record.getCancelType() != null) {
            SET("cancel_type = #{cancelType,jdbcType=INTEGER}");
        }
    ...
    protected void applyWhere(AppOrderExample example, boolean includeExamplePhrase) {
        ...
        List<AppOrderExample.Criteria> oredCriteria = example.getOredCriteria();
        boolean firstCriteria = true;
        for (int i = 0; i < oredCriteria.size(); i++) {
            AppOrderExample.Criteria criteria = oredCriteria.get(i);
            if (criteria.isValid()) {
                if (firstCriteria) {
                    firstCriteria = false;
                } else {
                ...
                sb.append('(');
                List<AppOrderExample.Criterion> criterions = criteria.getAllCriteria();
                boolean firstCriterion = true;
                for (int j = 0; j < criterions.size(); j++) {
                    AppOrderExample.Criterion criterion = criterions.get(j);
                    if (firstCriterion) {
