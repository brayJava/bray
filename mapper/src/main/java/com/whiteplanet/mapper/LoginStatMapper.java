...
public interface LoginStatMapper {
    ...
    @Select({
            ...
            "from login_stat",
            "where date_format(login_date, '%Y/%m/%d') = #{time,jdbcType=VARCHAR}"
    })
    Integer selectDailyActivity(String time);

    @Select({
            "select count(*) activity_count from (" ,
            "SELECT" ,
                    " DISTINCT doctor_id " ,
                    " FROM " ,
                    " login_stat" ,
                    " WHERE " ,
                    " where date_format(login_date, '%Y/%m/%d') >= #{startDate,jdbcType=VARCHAR} " ,
                    " and date_format(login_date, '%Y/%m/%d') <= #{endDate,jdbcType=VARCHAR} " ,
            ") id_count "
    })
    Integer selectWeekOrMonthActivity(String startDate ,String endDate);
