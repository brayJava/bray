...
public class PatientDepartmentIllnessSqlProvider {

    public String insertSelective(PatientDepartmentIllnessKey record) {
        SQL sql = new SQL();
        sql.INSERT_INTO("patient_department_illness");
    ...
    public String insertSelectives(Map<String,Object> params){
        Collection<PatientDepartmentIllnessKey> records = (Collection<PatientDepartmentIllnessKey>) params.get("records");
        StringBuilder sql = new StringBuilder();
        sql.append("insert into patient_department_illness(patient_recommend_doctor_id,label_notes_id)")
                .append("values");
        for(PatientDepartmentIllnessKey item:records){
            sql.append("(")
