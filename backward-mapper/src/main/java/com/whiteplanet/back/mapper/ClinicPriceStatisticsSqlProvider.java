...
public class ClinicPriceStatisticsSqlProvider {
    ...
    protected void applyWhere(ClinicPriceStatisticsExample example, boolean includeExamplePhrase) {
        ...
        List<ClinicPriceStatisticsExample.Criteria> oredCriteria = example.getOredCriteria();
        boolean firstCriteria = true;
        for (int i = 0; i < oredCriteria.size(); i++) {
            ClinicPriceStatisticsExample.Criteria criteria = oredCriteria.get(i);
            if (criteria.isValid()) {
                if (firstCriteria) {
                    firstCriteria = false;
                } else {
                ...
                sb.append('(');
                List<ClinicPriceStatisticsExample.Criterion> criterions = criteria.getAllCriteria();
                boolean firstCriterion = true;
                for (int j = 0; j < criterions.size(); j++) {
                    ClinicPriceStatisticsExample.Criterion criterion = criterions.get(j);
                    if (firstCriterion) {
