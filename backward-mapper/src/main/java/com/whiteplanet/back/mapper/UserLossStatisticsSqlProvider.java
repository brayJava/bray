...
public class UserLossStatisticsSqlProvider {
    ...
    protected void applyWhere(UserLossStatisticsExample example, boolean includeExamplePhrase) {
        ...
        List<UserLossStatisticsExample.Criteria> oredCriteria = example.getOredCriteria();
        boolean firstCriteria = true;
        for (int i = 0; i < oredCriteria.size(); i++) {
            UserLossStatisticsExample.Criteria criteria = oredCriteria.get(i);
            if (criteria.isValid()) {
                if (firstCriteria) {
                    firstCriteria = false;
                } else {
                ...
                sb.append('(');
                List<UserLossStatisticsExample.Criterion> criterions = criteria.getAllCriteria();
                boolean firstCriterion = true;
                for (int j = 0; j < criterions.size(); j++) {
                    UserLossStatisticsExample.Criterion criterion = criterions.get(j);
                    if (firstCriterion) {
