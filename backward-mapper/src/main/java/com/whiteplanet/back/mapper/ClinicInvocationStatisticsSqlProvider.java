...
public class ClinicInvocationStatisticsSqlProvider {
    ...
    protected void applyWhere(ClinicInvocationStatisticsExample example, boolean includeExamplePhrase) {
        ...
        List<ClinicInvocationStatisticsExample.Criteria> oredCriteria = example.getOredCriteria();
        boolean firstCriteria = true;
        for (int i = 0; i < oredCriteria.size(); i++) {
            ClinicInvocationStatisticsExample.Criteria criteria = oredCriteria.get(i);
            if (criteria.isValid()) {
                if (firstCriteria) {
                    firstCriteria = false;
                } else {
                ...
                sb.append('(');
                List<ClinicInvocationStatisticsExample.Criterion> criterions = criteria.getAllCriteria();
                boolean firstCriterion = true;
                for (int j = 0; j < criterions.size(); j++) {
                    ClinicInvocationStatisticsExample.Criterion criterion = criterions.get(j);
                    if (firstCriterion) {
